...
const (
	...
	// maximum number of simultaneous live-transcode streams permitted for a
	// single file. 0 (the default) means unlimited.
	MaxConcurrentStreamsPerScene = "max_concurrent_streams_per_scene"
	...
	// AI stack (opt-in, disabled by default). All external service URLs default
	// to the empty string; when unset the corresponding feature skips cleanly
	// and never contacts any host. No private/LAN addresses are hardcoded.
	//
	// subtitle (caption) generation via an external ASR service.
	// SubtitleGenerationURL is the base URL of the service exposing
	// POST /v1/audio/transcriptions. Empty (default) disables subtitle
	// generation. SubtitleGenerationLanguage is the language code sent as the
	// "prompt" field and stored as the caption language.
	...
	// SubtitleGenerationTranslate, when true, translates generated captions to
	// SubtitleGenerationTranslateTo via the service /v1/translate.
	...
	// Empty (default) disables dubbing. DubbingVoice is the reference voice
	// name the dub service should use.
...
// GetMaxConcurrentStreamsPerScene returns the maximum number of simultaneous
// live-transcode streams allowed for a single file. 0 means unlimited.
func (i *Config) GetMaxConcurrentStreamsPerScene() int {
	return i.getInt(MaxConcurrentStreamsPerScene)
}
...
// used to generate captions. Empty means subtitle generation is unconfigured
// and the task skips cleanly; no default host is assumed.
func (i *Config) GetSubtitleGenerationURL() string {
	return strings.TrimRight(i.getString(SubtitleGenerationURL), "/")
}
...
// GetSubtitleGenerationTranslate reports whether generated captions should be
// translated to GetSubtitleGenerationTranslateTo.
...
// captions are translated to (default Chinese).
...
// GetSubtitleGenerationDenoise reports whether generated captions should apply
...
// applied when SubtitleGenerationDenoise is enabled (default "afftdn").
...
// GetDubbingURL returns the base URL of the dub service. Empty means dubbing
// is unconfigured and the task skips cleanly; no default host is assumed.
func (i *Config) GetDubbingURL() string {
	return strings.TrimRight(i.getString(DubbingURL), "/")
}
...
// GetOllamaEnabled reports whether the Ollama dictionary service is enabled.
func (i *Config) GetOllamaEnabled() bool {
	return i.getBoolDefault(OllamaEnabled, ollamaEnabledDefault)
}

// GetOllamaBaseURL returns the configured Ollama base URL. Empty means the
...
// GetOllamaTimeout returns the Ollama request timeout in milliseconds.
func (i *Config) GetOllamaTimeout() int {
	ret := i.getInt(OllamaTimeout)
	if ret <= 0 {
...
// GetOllamaFallbackToTraditionalDict reports whether the client should fall
// back to the traditional dictionary when the AI service is unavailable.
func (i *Config) GetOllamaFallbackToTraditionalDict() bool {
...
func (i *Config) setDefaultValues() {
	...
	// AI stack defaults. Service URLs are intentionally left unset (empty) so
	// the features stay opt-in and never contact any host until configured.
	i.setDefault(SubtitleGenerationLanguage, subtitleGenerationLangDefault)
	i.setDefault(SubtitleGenerationTranslate, subtitleGenerationTranslateDefault)
	i.setDefault(SubtitleGenerationTranslateTo, subtitleGenerationTranslateToDefault)
	...
	i.setDefault(OllamaEnabled, ollamaEnabledDefault)
