...
func (t *ImportTask) unzipFile() error {
	...
	for _, f := range r.File {
		...
		// Zip Slip mitigation: a malicious archive entry name such as
		// "../../etc/passwd" would otherwise resolve outside BaseDir. Skip any
		// entry whose destination escapes the extraction directory.
		if !fsutil.IsPathInDir(t.BaseDir, fn) {
			logger.Warnf("skipping zip entry %q: path escapes destination directory", f.Name)
			continue
		}
