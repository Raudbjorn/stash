...
// performerReader/studioReader/tagReader return the relevant reader when that
// category is enabled, else nil (so match.Cache.Warm can skip it).
func (t *autoTagFilesTask) performerReader(r models.Repository) models.PerformerAutoTagQueryer {
	if t.performers {
		return r.Performer
	}
	return nil
...
func (t *autoTagFilesTask) studioReader(r models.Repository) models.StudioAutoTagQueryer {
	if t.studios {
		return r.Studio
	}
	return nil
...
func (t *autoTagFilesTask) tagReader(r models.Repository) models.TagAutoTagQueryer {
	if t.tags {
		return r.Tag
	}
	return nil
...
// autoTagWorkers returns the number of concurrent read-phase (matching) workers
// to use for file-based auto-tagging. The write phase remains serial because
// stash uses a single write connection.
func autoTagWorkers() int {
	n := 1
	if instance != nil && instance.Config != nil {
		n = instance.Config.GetParallelTasksWithAutoDetection()
	}
	if n < 1 {
		n = 1
	}
...
// collectAndApply runs the read-only matching phase for each item concurrently
// (bounded by workers), then applies the resulting links serially. Matching is
// the expensive, CPU-bound part and parallelises across the read-connection
// pool; writes serialise on stash's single write connection regardless, so they
// are applied in a simple serial pass which also keeps progress deterministic.
func collectAndApply[T any](
	ctx context.Context,
	items []T,
	workers int,
	progress *job.Progress,
	collect func(ctx context.Context, item T) (autotag.MatchResult, error),
	apply func(ctx context.Context, item T, res autotag.MatchResult) error,
) {
	results := make([]autotag.MatchResult, len(items))
	...
	for i := range items {
		...
		go func(i int) {
			defer wg.Done()
			defer func() { <-sem }()
			results[i], errs[i] = collect(ctx, items[i])
		}(i)
	}
	wg.Wait()
	...
	for i := range items {
		...
		case errs[i] != nil:
			if !job.IsCancelled(ctx) {
				logger.Errorf("auto-tag error: %v", errs[i])
			}
		case !results[i].Empty():
			if err := apply(ctx, items[i], results[i]); err != nil {
				if !job.IsCancelled(ctx) {
					logger.Errorf("auto-tag error: %v", err)
				}
			}
...
func (t *autoTagFilesTask) processScenes(ctx context.Context) {
	...
	for more {
		...
		collectAndApply(ctx, scenes, workers, t.progress,
			func(ctx context.Context, ss *models.Scene) (autotag.MatchResult, error) {
				var res autotag.MatchResult
				err := r.WithReadTxn(ctx, func(ctx context.Context) error {
					var e error
					res, e = autotag.CollectSceneMatches(ctx, ss, r.Performer, r.Studio, r.Tag, t.cache, t.performers, t.studios, t.tags)
					return e
				})
				return res, err
			},
			func(ctx context.Context, ss *models.Scene, res autotag.MatchResult) error {
				return r.WithTxn(ctx, func(ctx context.Context) error {
					return autotag.ApplySceneMatches(ctx, ss, r.Scene, res)
				})
			},
		)
...
func (t *autoTagFilesTask) processImages(ctx context.Context) {
	...
	for more {
		...
		collectAndApply(ctx, images, workers, t.progress,
			func(ctx context.Context, ss *models.Image) (autotag.MatchResult, error) {
				var res autotag.MatchResult
				err := r.WithReadTxn(ctx, func(ctx context.Context) error {
					var e error
					res, e = autotag.CollectImageMatches(ctx, ss, r.Performer, r.Studio, r.Tag, t.cache, t.performers, t.studios, t.tags)
					return e
				})
				return res, err
			},
			func(ctx context.Context, ss *models.Image, res autotag.MatchResult) error {
				return r.WithTxn(ctx, func(ctx context.Context) error {
					return autotag.ApplyImageMatches(ctx, ss, r.Image, res)
				})
			},
		)
...
func (t *autoTagFilesTask) processGalleries(ctx context.Context) {
	...
	for more {
		...
		collectAndApply(ctx, galleries, workers, t.progress,
			func(ctx context.Context, ss *models.Gallery) (autotag.MatchResult, error) {
				var res autotag.MatchResult
				err := r.WithReadTxn(ctx, func(ctx context.Context) error {
					var e error
					res, e = autotag.CollectGalleryMatches(ctx, ss, r.Performer, r.Studio, r.Tag, t.cache, t.performers, t.studios, t.tags)
					return e
				})
				return res, err
			},
			func(ctx context.Context, ss *models.Gallery, res autotag.MatchResult) error {
				return r.WithTxn(ctx, func(ctx context.Context) error {
					return autotag.ApplyGalleryMatches(ctx, ss, r.Gallery, res)
				})
			},
		)
