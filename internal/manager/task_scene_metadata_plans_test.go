...
func TestPurgeSceneMetadataPlansKeepsSiblingRunActions(t *testing.T) {
	repository := newTestRepository(t)
	kept := createTestScene(t, repository, "Kept Scene")
	purged := createTestScene(t, repository, "Purged Scene")
	job := &analyzeSceneMetadataJob{repository: repository}
	runID := "shared-run"
	...
	for _, plan := range []*AnalysisPlan{
		{
			...
			State:          SceneMetadataPlanProposed, CreatedAt: old,
			Suggested: []SuggestedField{{
		...
		},
		{
			...
			State:          SceneMetadataPlanApplied, CreatedAt: old,
			Suggested: []SuggestedField{{
				Kind: sceneMetadataActionTitle, PayloadJSON: actionPayload(sceneMetadataActionPayload{Title: &title}),
	...
	if record := findSceneMetadataPlanRecord(t, repository, runID, kept.ID); record.State != string(SceneMetadataPlanProposed) {
	...
	if len(actions) != 1 || actions[0].State != string(SceneMetadataPlanProposed) {
