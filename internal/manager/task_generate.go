...
type GenerateMetadataInput struct {
	...
	// Subtitles generates captions for videos without them via the configured
	// ASR service. Opt-in; a no-op when no service is configured.
	Subtitles bool `json:"subtitles"`
	// SubtitleLanguage overrides the configured language for this run (ASR
	// prompt + caption label).
	SubtitleLanguage *string `json:"subtitleLanguage"`
	// Dubbing generates a dubbed video (sidecar .<lang>-dub.mp4) from a scene's
	// translated caption via the configured dub service. Opt-in; a no-op when
