...
type Resolver struct {
	...
	// ollamaService is the (opt-in) Ollama dictionary client. It is lazily
	// constructed from the config system by getOllamaService.
	ollamaService *ollama.Service
}

// getOllamaService returns an Ollama service built from the current config
// system values. The service stays disabled/inert until the user configures
// and enables Ollama; no host is auto-detected.
func (r *Resolver) getOllamaService() *ollama.Service {
	...
	oc.FallbackToTraditionalDict = c.GetOllamaFallbackToTraditionalDict()
	oc.MistralAPIKey = c.GetMistralAPIKey()

	if r.ollamaService == nil {
		r.ollamaService = ollama.NewService(oc)
	} else {
		r.ollamaService.UpdateConfig(oc)
	}
