...
func (r *mutationResolver) MovieCreate(ctx context.Context, input MovieCreateInput) (*models.Group, error) {
	...
	if input.Aliases != nil {
		var aliases []string
		if alias := strings.TrimSpace(*input.Aliases); alias != "" {
			aliases = stringslice.UniqueExcludeFold([]string{alias}, newGroup.Name)
		}
		newGroup.Aliases = models.NewRelatedStrings(aliases)
	}
