...
func (r *sceneResolver) URL(ctx context.Context, obj *models.Scene) (*string, error) {
	urls, err := r.getSceneURLs(ctx, obj)
	if err != nil {
		return nil, err
	}
...
func (r *sceneResolver) Urls(ctx context.Context, obj *models.Scene) ([]string, error) {
	return r.getSceneURLs(ctx, obj)
}

// getSceneURLs returns the scene's URLs, preferring an eager-loaded value and
// otherwise batching the lookup through the SceneURLs dataloader to avoid an
// N+1 query when resolving lists of scenes.
func (r *sceneResolver) getSceneURLs(ctx context.Context, obj *models.Scene) ([]string, error) {
	if obj.URLs.Loaded() {
		return obj.URLs.List(), nil
	}

	return loaders.From(ctx).SceneURLs.Load(obj.ID)
}
