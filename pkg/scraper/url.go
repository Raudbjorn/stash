...
const (
	// scrapeMaxRetries is the number of times a request is retried after
	// receiving an HTTP 429 (Too Many Requests) response.
	scrapeMaxRetries = 3
	// scrapeRetryBackoff is the initial backoff duration between retries; it
	// doubles after each attempt (exponential backoff).
	scrapeRetryBackoff = time.Second * 2
)

var proxyAuthRE = regexp.MustCompile(`^(https?:\/\/)(([\P{Cc}]+):([\P{Cc}]+)@)?(([a-zA-Z0-9][a-zA-Z0-9.-]*)(:[0-9]{1,5})?)`)

// doWithBackoff performs the request, retrying on HTTP 429 (Too Many Requests)
// with exponential backoff. When the response carries a Retry-After header it is
// honoured in preference to the computed backoff. The request must be safe to
// retry (no consumed body) - all scraper requests here are GETs with a nil body.
func doWithBackoff(ctx context.Context, client *http.Client, req *http.Request) (*http.Response, error) {
	...
	for attempt := 0; ; attempt++ {
		...
		if resp.StatusCode != http.StatusTooManyRequests || attempt >= scrapeMaxRetries {
			return resp, nil
		}
		...
		logger.Warnf("[scraper] HTTP 429 from %s, retrying in %s (attempt %d/%d)", req.URL, wait, attempt+1, scrapeMaxRetries)
...
// number of seconds (delta) or an HTTP date. It returns 0 if the value is
