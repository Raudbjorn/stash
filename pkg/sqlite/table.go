...
// getMany returns the ordered values for each of the given ids, keyed by id.
// It issues a single query, enabling dataloader-style batching (avoiding N+1).
func (t *orderedValueTable[T]) getMany(ctx context.Context, ids []int) (map[int][]T, error) {
	q := dialect.Select(t.idColumn, t.valueColumn).From(t.table.table).
