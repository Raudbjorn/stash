...
// GetManyURLs returns the URLs for each of the given scene ids, in the same
// order as ids. It batches into a single query to avoid N+1 lookups.
func (qb *SceneStore) GetManyURLs(ctx context.Context, ids []int) ([][]string, error) {
	byID, err := scenesURLsTableMgr.getMany(ctx, ids)
	if err != nil {
		return nil, err
	}
