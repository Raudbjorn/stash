...
func (sm *StreamManager) ServeTranscode(w http.ResponseWriter, r *http.Request, options TranscodeOptions) {
	// enforce the per-file concurrent live-transcode limit
	streamKey := options.VideoFile.Path
	if !sm.acquireStreamSlot(streamKey) {
		logger.Warnf("[transcode] concurrent stream limit reached for %s", streamKey)
		w.WriteHeader(http.StatusTooManyRequests)
		if _, err := w.Write([]byte("too many concurrent streams for this file")); err != nil {
			logger.Warnf("[transcode] error writing response: %v", err)
		}
		return
	}
	defer sm.releaseStreamSlot(streamKey)
