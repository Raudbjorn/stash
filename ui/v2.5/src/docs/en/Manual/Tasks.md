...
### Legacy interactive heatmaps

Removing Funscript support does not delete heatmap PNGs generated by older versions. The cleaning task cannot identify these files after upgrading. To reclaim the space, stop Stash and manually remove the `interactive_heatmaps` directory from the configured generated-files directory.
