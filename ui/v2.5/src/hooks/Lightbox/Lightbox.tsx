...
export const LightboxComponent: React.FC<IProps> = ({
  ...
  // The lightbox pauses the global Mousetrap while visible (see below), so the
  // rating keybinds bind to their own scoped instance instead.
  const mousetrap = useMemo(() => new Mousetrap(), []);

  const setImageRating = useCallback(
    (v: number | null) => {
      const currentImage = images[index === null ? initialIndex : index];
      if (currentImage?.id) {
        updateImage({
          variables: { input: { id: currentImage.id, rating100: v } },
        });
      }
    },
    [images, index, initialIndex, updateImage]
  );

  const setImageFavorite = useCallback(
    (v: boolean) => {
      const currentImage = images[index === null ? initialIndex : index];
      if (currentImage?.id) {
        updateImage({
          variables: { input: { id: currentImage.id, favorite: v } },
        });
      }
    },
    [images, index, initialIndex, updateImage]
  ...
  useRatingKeybinds(
    ...
    (v) => setImageRating(Number.isNaN(v) ? null : v),
    mousetrap
  );
          ...
          <div className={CLASSNAME_RIGHT}>
            ...
            {currentImage?.id && (
              <FavoriteIcon
                ...
                onToggleFavorite={(v) => setImageFavorite(v)}
              />
            )}
            {slideshowEnabled && (
              <Button
          ...
          <div className={CLASSNAME_FOOTER_LEFT}>
            {currentImage?.id !== undefined && (
              <RatingSystem
                ...
                onSetRating={(v) => setImageRating(v)}
                clickToRate
                withoutContext
              />
            )}
