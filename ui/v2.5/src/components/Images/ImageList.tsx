...
// preview (clip animation) takes priority when present; otherwise, prefer the
// original image over the thumbnail for small files (<1MB) to avoid a second
// round of lossy compression
function getImageSrc(image: GQL.SlimImageDataFragment): string {
  if (image.paths.preview) {
    return image.paths.preview;
  }

  const fileSize = image.visual_files?.[0]?.size ?? 0;
  const hasImagePath = !!image.paths.image;
  const hasThumbnailPath = !!image.paths.thumbnail;

  if (fileSize < 1048576 && hasImagePath) {
