...
const CustomFilterResults: React.FC<ICustomFilterProps> = ({
  ...
  const filter = useMemo(() => {
    // guard against malformed front page entries (e.g. a saved CustomFilter
    // missing its mode) so one bad row doesn't crash the whole front page
    if (!customFilter.mode || !getFilterOptions(customFilter.mode)) {
      return undefined;
    }
